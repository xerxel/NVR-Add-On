...
The in-app channel editor has exactly eight slots, all enabled by default for a new configuration. Each holds enabled state, display name, NVR channel number, motion entity, camera entity, main/sub track IDs, and an optional timestamp offset. New configurations default to `binary_sensor.network_video_recorder_channel_N_motion` and `camera.network_video_recorder_channel_N`, with the appropriate channel number. Use **Diagnostics > Entity discovery** to confirm or replace those IDs. Mappings are atomically stored in `/data/timeline/channels.json`; upgrades preserve existing enabled/disabled choices.
